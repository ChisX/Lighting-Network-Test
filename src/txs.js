...
function seralizeObjVal(obj) {
    ...
    let bin = Object.keys(obj).reduce((result, key) => {
...
function dsha256(data) {
    ...
    let tmp = crypto.createHash('sha256').update(bytes).digest();
    let hash = crypto.createHash('sha256').update(tmp).digest();
    return hash;
}
...
function ecdsa_sign(tx, priv) {
    ...
    let dhash = dsha256(tx);

    // Extract out the 256 bit priv key and convert to bytes
    let key = Buffer.from(keys.decodePrivKey(priv), 'hex');
...
async function signInput(tx, indx, wallet) {
    ...
    let lockingKeyHash = tx.inputs[indx].unlockScript.slice(6,46);
    let pubKeyHash = keys.getKeyHashFromAddr(wallet.address);
    ...
    let binTx = seralizeObjVal(newtx);
    let signature = await ecdsa_sign(binTx, wallet.privateKey);
    signature = signature.toString('hex') + '01'; //SIGHASH_ALL

    // 4. Create the unlock script and insert it to the tx.inputs[indx]
    let sigLenInBytes = toLE(addPadding((signature.length/2).toString(16), 1));
    let pubKeyLenInBytes = toLE(addPadding((wallet.publicKey.length/2).toString(16), 1));
    let unlockScript = sigLenInBytes + signature + pubKeyLenInBytes + wallet.publicKey;
...
function createSingleOutput(amount, toAddr) {
    ...
    let pubKeyHash = keys.getKeyHashFromAddr(toAddr);
    let keyHashInBytes = (pubKeyHash.length/2).toString(16); // # of bytes
    let script = opcodes.OP_DUP + opcodes.OP_HASH160 + keyHashInBytes
...
function createOutputs(amount, toAddr, inputValue, wallet) {
    ...
    let change = inputValue - amount - getFee();
    if (change > 0) {
...
async function create(utxo, amount, toAddr, wallet) {
    let inputs = createInputs(utxo, amount);
    let inputValue = inputs.pop();
    let outputs = createOutputs(amount, toAddr, inputValue, wallet);
    let tx = getNewTx(inputs, outputs);
